package org.example.thirdpartyapiinterceptor.postservice.impl;


import lombok.val;
import org.example.thirdpartyapiinterceptor.postservice.PostService;
...
public class PostServiceImpl implements PostService {
   ...
   private RestTemplate   restTemplate;
    ...
    @Override
    public Map<String, Object> callThirdPartyAPI(String name) {
        HttpEntity <Map> httpEntity  = new HttpEntity<>(gethttpHeaders(name));
        String url = "http://localhost:8080/thirdPartyAPI";
        val response  =  restTemplate.exchange(url , HttpMethod.GET ,httpEntity,Map.class); ;
        return  response.getBody();
    }
    ...
    private HttpHeaders gethttpHeaders(String name){
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
        headers.setContentType(MediaType.APPLICATION_JSON);
